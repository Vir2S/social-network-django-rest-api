...
class SocialBot(object):
    ...
    def signup_users(self):
        ...
        while number_of_users:
            ...
            if not response.status_code == status.HTTP_201_CREATED:
                raise ex.BotSignUpException(response.content)
    ...
    def login_users(self):
        ...
        for user in self.signed_up_users:
            ...
            if not response.status_code == status.HTTP_200_OK:
                raise ex.BotLoginException(response.content)

            self.logged_users.append(json.loads(response.content))

    def create_posts(self):
        """
            Create random numbe, no greater then max_posts_per_user, for every user. Every post will be
        ...
        for user in self.logged_users:
            ...
            while rand_posts:
                ...
                if not response.status_code == status.HTTP_201_CREATED:
                    raise ex.BotCreatePostException(response.content)
    ...
    def like_posts(self, post_id, user):
        ...
        if not response.status_code == status.HTTP_201_CREATED:
            raise ex.BotLikeException(response.content)
            ...
            for post in posts:
                if post['user'] != next_top_user['user']['pk'] and \
                   post['id'] not in next_top_user['posts_liked']:
                    no_maxusr_posts.append(post)

            post = random.choice(no_maxusr_posts)
